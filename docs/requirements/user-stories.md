...
**<a id="US-019">US-019</a>**

Як користувач, я хочу бачити повідомлення про успішні дії (створення замовлення, скасування, додавання улюбленого), щоб розуміти, що система відреагувала.

**Критерії прийняття:**

 - [ ] Якщо я користувач, коли створюю замовлення, тоді бачу повідомлення «Замовлення успішно створено».
 - [ ] Якщо я користувач, коли додаю або видаляю страву з улюбленого, тоді бачу коротке підтвердження.
 - [ ] Якщо я користувач, коли скасовую замовлення, тоді бачу підтвердження дії.
...
- **<a id="US-019">US-019</a>**
  - Independent: Так - не залежить від інших історій; базується на вже існуючих діях (створення замовлення, лайк/анлайк, скасування).
  - Negotiable: Так - можна варіювати тексти, місце появи, тривалість, тип (toast/alert), іконки.
  - Valuable: Так - підвищує зрозумілість і відчуття контролю в користувача, зменшує повторні кліки й звернення в підтримку.
  - Estimable: Так - чітко оцінюється як невелике фронт-завдання без змін у бекенді.
  - Small: Так - мікрофіча (кілька повідомлень у 2–3 ключових потоках).
  - Testable: Так - e2e/юнiт:
  показ тосту після відповіді 2xx,
  відсутність/інший меседж при 4xx/5xx,
  доступність через `aria-live`/фокус.
